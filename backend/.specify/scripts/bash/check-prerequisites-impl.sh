...
# check-prerequisites-impl.sh - Check prerequisites for implementation phase and return feature directory and available docs
...
if [[ ! -f "${FEATURE_DIR}tasks.md" ]]; then
    echo "tasks.md not found in ${FEATURE_DIR}" >&2
    exit 1
fi
...
# Include checklists if they exist
if [[ -d "${FEATURE_DIR}checklists" ]]; then
    for checklist in "${FEATURE_DIR}checklists/"*; do
        if [[ -f "$checklist" ]]; then
            AVAILABLE_DOCS+=("$checklist")
        fi
    done
fi
